#!/usr/bin/env python3
"""输入数据校验脚本 - insurance-underwriting-health-verification
...
def validate(data: dict) -> list[str]:
    ...
    # 投保人姓名
    if not data.get("投保人姓名"):
        errors.append("缺少必填字段：投保人姓名")

    # 被保人年龄
    age = data.get("被保人年龄")
    if age is None:
        errors.append("缺少必填字段：被保人年龄")
    elif not isinstance(age, (int, float)) or age < 0 or age > 70:
        errors.append("被保人年龄必须在 0-70 岁之间")

    # 申请险种
    if not data.get("申请险种"):
        errors.append("缺少必填字段：申请险种")

    # 保额
    amount = data.get("保额")
    if amount is None:
        errors.append("缺少必填字段：保额")
